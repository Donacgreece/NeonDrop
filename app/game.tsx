...
export function Game(){
  ...
  const begin=useCallback(()=>{
    ...
    setName(safe);localStorage.setItem("neon-drop-name",safe);scoreRef.current=0;livesRef.current=3;setScore(0);setLives(3);runRef.current+=1;sync("countdown");
  ...
  useEffect(()=>{
    ...
    let raf=0,last=performance.now(),width=0,height=0,dpr=1,seenRun=-1,invincible=0,shake=0;
    let player={x:0,y:0,r:12,vy:0};
    ...
    const reset=()=>{player={x:width/2,y:height*.27,r:Math.max(11,width*.03),vy:0};targetXRef.current=width/2;gates=[];for(let i=0;i<7;i++)gates.push(makeGate(height*(.62+i*.18),i));particles=[];invincible=0};
    ...
    const finish=()=>{shake=14;burst(player.x,player.y,"#ff4ea3",34);saveScore(scoreRef.current);sync("over")};
    const hitGate=(g:typeof gates[number])=>{
      ...
      if(livesRef.current<=0){finish();return} invincible=1.1;player.vy=-300;
    };
    const frame=(now:number)=>{
      const dt=Math.min(.033,(now-last)/1000);last=now;
      if(seenRun!==runRef.current){seenRun=runRef.current;reset()}
      if(stateRef.current==="playing"){
        invincible=Math.max(0,invincible-dt);player.x+=(targetXRef.current-player.x)*Math.min(1,dt*13);player.vy+=430*dt;player.y+=player.vy*dt;
        const speed=Math.min(205,58+scoreRef.current*4);gates.forEach(g=>g.y-=speed*dt);
        const bottom=player.y+player.r;
        for(const g of gates){if(!g.passed&&player.vy>0&&bottom>=g.y-5&&bottom-player.vy*dt<g.y+10){const safe=player.x-player.r>g.gapX&&player.x+player.r<g.gapX+g.gapW;if(safe){g.passed=true;player.vy=-325;scoreRef.current+=1;setScore(scoreRef.current);burst(player.x,g.y,g.color,9)}else if(invincible<=0)hitGate(g)}}
    ...
    const aim=(e:PointerEvent)=>{if(stateRef.current!=="playing")return;const rect=canvas.getBoundingClientRect();targetXRef.current=Math.max(18,Math.min(rect.width-18,e.clientX-rect.left))};
    ...
    resize();window.addEventListener("resize",resize);window.addEventListener("pointerdown",aim);window.addEventListener("pointermove",aim);window.addEventListener("keydown",key);raf=requestAnimationFrame(frame);
    return()=>{cancelAnimationFrame(raf);window.removeEventListener("resize",resize);window.removeEventListener("pointerdown",aim);window.removeEventListener("pointermove",aim);window.removeEventListener("keydown",key)}
  },[]);
  ...
  return <main className="game-shell"><section className="game-frame" aria-label="Neon Drop game">
    <canvas ref={canvasRef} aria-hidden="true"/><header className="hud"><div className="logo">NEON <span>DROP</span></div><div className="score-wrap"><strong className="score">{pad(score)}</strong><span className="best">BEST {pad(best)}</span></div></header>
